...
M1 Database name: wam1_nonprod_delta_2
...
SELECT ce.entity_id, ce.email, ce.group_id, ce.is_active, ce.created_at FROM wam1_nonprod_delta_2.customer_entity AS ce
WHERE ce.updated_at > '2021-09-19 21:36:21'
ORDER BY updated_at;
...
SELECT cev.entity_id, ea.attribute_code, cev.value FROM wam1_nonprod_delta_2.customer_entity_varchar AS cev
INNER JOIN wam1_nonprod_delta_2.eav_attribute AS ea ON ea.attribute_id = cev.attribute_id AND ea.entity_type_id = 1 AND ea.frontend_input NOT IN ('select', 'multiselect') AND ea.attribute_code NOT IN ( 'taxvat', 'no_of_jobs_per_year', 'type_of_projects')
...
INSERT INTO wendover_m2_live.perficient_customer_attribute_temp (entity_id, attribute_code, VALUE)
SELECT cev.entity_id, ea.attribute_code, eaov.value FROM wam1_nonprod_delta_2.customer_entity_varchar AS cev
INNER JOIN wam1_nonprod_delta_2.eav_attribute AS ea ON ea.attribute_id = cev.attribute_id AND ea.entity_type_id = 1 AND
ea.frontend_input = 'select'
INNER JOIN wendover_m2_live.perficient_customer_temp AS pct ON pct.entity_id = cev.entity_id
INNER JOIN wam1_nonprod_delta_2.eav_attribute_option_value AS eaov ON eaov.option_id = cev.value;
...
FROM wam1_nonprod_delta_2.customer_entity_varchar AS cei
INNER JOIN wam1_nonprod_delta_2.eav_attribute AS ea ON ea.attribute_id = cei.attribute_id AND ea.entity_type_id = 1 AND ea.attribute_code = 'des_comm'
INNER JOIN wendover_m2_live.perficient_customer_temp AS pct ON pct.entity_id = cei.entity_id
INNER JOIN wam1_nonprod_delta_2.eav_attribute_option_value AS eaov
...
INSERT INTO wendover_m2_live.perficient_customer_attribute_temp (entity_id, attribute_code, VALUE)
SELECT cet.entity_id, ea.attribute_code, cet.value FROM wam1_nonprod_delta_2.customer_entity_text AS cet
INNER JOIN wam1_nonprod_delta_2.eav_attribute AS ea ON ea.attribute_id = cet.attribute_id AND ea.entity_type_id = 1
...
INSERT INTO wendover_m2_live.perficient_customer_attribute_temp (entity_id, attribute_code, VALUE)
SELECT cei.entity_id, ea.attribute_code, cei.value FROM wam1_nonprod_delta_2.customer_entity_int AS cei
INNER JOIN wam1_nonprod_delta_2.eav_attribute AS ea ON ea.attribute_id = cei.attribute_id AND ea.entity_type_id = 1 AND ea.attribute_code NOT IN( 'default_billing', 'default_shipping', 'password_created_at', 'reward_update_notification', 'reward_warning_notification', 'sales_rep', 'gender', 'customer_activated')
INNER JOIN wendover_m2_live.perficient_customer_temp AS pct ON pct.entity_id = cei.entity_id;

/** Insert data into M2 perficient_customer_attribute_temp from M1 customer_entity_int table for select attributes  **/
INSERT INTO wendover_m2_live.perficient_customer_attribute_temp (entity_id, attribute_code, VALUE)
SELECT cei.entity_id, ea.attribute_code, eaov.value FROM wam1_nonprod_delta_2.customer_entity_int AS cei
INNER JOIN wam1_nonprod_delta_2.eav_attribute AS ea ON ea.attribute_id = cei.attribute_id AND ea.entity_type_id = 1 AND ea.attribute_code IN( 'sales_rep', 'gender')
INNER JOIN wendover_m2_live.perficient_customer_temp AS pct ON pct.entity_id = cei.entity_id
INNER JOIN wam1_nonprod_delta_2.eav_attribute_option_value AS eaov ON eaov.option_id = cei.value;

/** Insert data into M2 perficient_customer_attribute_temp from M1 customer_entity_int table for boolean attributes  **/
INSERT INTO wendover_m2_live.perficient_customer_attribute_temp (entity_id, attribute_code, VALUE)
SELECT cei.entity_id, ea.attribute_code, if (cei.value = 0, 'No', 'Yes') as value FROM wam1_nonprod_delta_2.customer_entity_int AS cei
INNER JOIN wam1_nonprod_delta_2.eav_attribute AS ea ON ea.attribute_id = cei.attribute_id AND ea.entity_type_id = 1 AND ea.attribute_code = 'customer_activated'
INNER JOIN wendover_m2_live.perficient_customer_temp AS pct ON pct.entity_id = cei.entity_id;
...
INSERT INTO wendover_m2_live.perficient_customer_attribute_temp (entity_id, attribute_code, VALUE)
SELECT cev.entity_id, 'taxvat', cev.value FROM wam1_nonprod_delta_2.customer_entity_varchar AS cev
INNER JOIN wam1_nonprod_delta_2.eav_attribute AS ea ON ea.attribute_id = cev.attribute_id AND ea.entity_type_id = 1 AND ea.attribute_code = 'tax_id'
INNER JOIN wendover_m2_live.perficient_customer_temp AS pct ON pct.entity_id = cev.entity_id;
...
FROM `wam1_nonprod_delta_2`.`customer_address_entity` AS custAdd
...
WHERE custAdd.updated_at > '2021-09-19 21:36:21';
...
UPDATE wendover_m2_live.customer_entity AS ce, (SELECT cei.*
	FROM wam1_nonprod_delta_2.eav_attribute AS ea
	INNER JOIN wam1_nonprod_delta_2.customer_entity_int AS cei ON ea.attribute_id = cei.attribute_id
	...
	AND ce1.updated_at > '2021-09-19 21:36:21'
...
AND ce.updated_at > '2021-09-19 21:36:21';
...
FROM `wam1_nonprod_delta_2`.grandriver_cc_customers AS gcc
...
SELECT ow.*,'' AS collaboration_ids FROM `wam1_nonprod_delta_2`.`wishlist` AS ow
INNER JOIN `wendover_m2_live`.`customer_entity` AS customer ON customer.entity_id = ow.customer_id
WHERE DATE(ow.updated_at) > '2021-09-19 21:51:11'
...
SELECT owi.* FROM `wam1_nonprod_delta_2`.`wishlist_item` AS owi
...
WHERE DATE(ww.updated_at) > '2021-09-19 21:51:11' AND owi.store_id != 2
...
SELECT owip.* FROM `wam1_nonprod_delta_2`.`wishlist_item_option` AS owip
...
WHERE DATE(ww.updated_at) > '2021-09-19 21:51:11' AND owip.`code` = 'info_buyRequest'
...
WHERE `option_id`
NOT IN (SELECT owip.`option_id` FROM `wam1_nonprod_delta_2`.`wishlist_item_option` AS owip);
...
NOT IN (SELECT `wishlist_item_id` FROM `wam1_nonprod_delta_2`.`wishlist_item`);
...
WHERE `wishlist_id`
NOT IN (SELECT `wishlist_id` FROM `wam1_nonprod_delta_2`.`wishlist`);
...
Replace wam1_nonprod_delta_2 DB name with actual M1 delta DB name
 */
...
DELETE wcgc FROM `wam1_nonprod_delta_2`.wendover_customer_gallery_catalog AS wcgc
LEFT JOIN `wendover_m2_live`.wishlist AS wl ON wl.wishlist_id = wcgc.wishlist_id
WHERE wl.wishlist_id IS NULL;

DELETE FROM `wam1_nonprod_delta_2`.wendover_customer_gallery_catalog WHERE customer_id IS NULL;

DELETE FROM `wam1_nonprod_delta_2`.`wendover_customer_gallery_catalog_page` WHERE catalog_id IS NULL;
...
FROM `wam1_nonprod_delta_2`.wendover_customer_gallery_catalog;

DELETE wcgc FROM wam1_nonprod_delta_2.`wendover_customer_gallery_catalog_page` AS wcgc
...
FROM `wam1_nonprod_delta_2`.`wendover_customer_gallery_catalog_page`;
/************************************************************************
