 ...
 *
 * @category: Magento
 * @package: Perficient/Multiple Wishlist
 * @copyright: Copyright  - 2020 Magento, Inc. All rights reserved.
 * See COPYING.txt for license details.
 * @license: Magento Enterprise Edition (MEE) license
 * @author: purushottam rathi <purushottam.rathi@Perficient.com>
 * @project: Wendover
 * @keywords: Module Multiple Wishlist WCAG Focus changes
 */
...
define([
    ...
    'mage/dataPost',
    'mage/dropdowns'
], function ($, mageTemplate, urlValidator, alert, confirm, $t, modal) {
    'use strict';
    $.widget('mage.multipleWishlist', {
        options: {
            ...
            createTmplData: {
                ...
                btnCloseClass: 'close',
                focusElement: '[data-role="closeBtn"]',
        ...
        _create: function () {
            ...
            // this will trigger the create new project popup if the pages coming from create a project link
            if(this.options.pageAction == 'create') {
                $('[data-wishlist-create]').click();
            }
        },
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (typeof response['wishlist_id'] !== 'undefined') {
                        let params = _this._wishlistProductData;
                        if (params) {
                            let url = params.action;
                            let wishListId = response['wishlist_id'];
                            let pageType = params.data.page_type;
                            let uenc = params.data.uenc;
                            let product = params.data.product;
                            let item = params.data.item;
                            let Pzcartproperties = params.data.pz_cart_properties;
                            let formKey = $('input[name="form_key"]').val();

                            let cartproperty = JSON.parse(Pzcartproperties);
                            let wishListProperty = {product: product, item:item, uenc: uenc, wishlist_id: wishListId, page_type: pageType, pz_cart_properties:Pzcartproperties, form_key: formKey, new_wishlist: true, ajax: 1};
                            if(cartproperty.CustomImage)    {
                                wishListProperty['edit_id'] = 1;
                            }
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _addToNew: function (data) {
            this._callback = $.proxy(function (wishlistId) {
                ...
                var pz_cart_properties = data.data['pz_cart_properties'];
                if($('#wishlist-success-content').length<=0) {
                    $('body').append('<div id="wishlist-success-content"></div>');
                }
                ...
                if($('body').is('.catalog-category-view, .catalogsearch-result-index ')) {
                    $.ajax({
                        ...
                        var successMessage = data.message + ' has been added successfully.';
                        if (data.relatedItems){
                            successMessage +=  '<br> <span><a href="#" id="' +
                                product + '"class="action product-set-wishlist">ADD COLLECTION</a></span>';
                        }

                        $('#wishlist-success-content').html(successMessage);
            ...
            if (this.options.wishlists && this.options.wishlists.length > 0) {
                $(this.options.wishlistLink).each($.proxy(function (index, e) {
                    ...
                    for (i = 0; i < this.options.wishlists.length; i++) {
                        var page_type = '';
                        if(this.options.wishlists[i].page_type && this.options.wishlists[i].page_type.length > 0) {
                            //alert(this.options.wishlists[i].page_type);
                            page_type = this.options.wishlists[i].page_type;
                        }
                    ...
                    element.remove();
                    $('body.catalog-product-view').trigger('selectionState-update');
                }, this));
            }
        }
    });
