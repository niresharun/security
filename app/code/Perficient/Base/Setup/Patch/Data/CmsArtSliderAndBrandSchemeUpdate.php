<?php
/**
 * This file is used to Update Source of Art Slider Homepage block and Brand Scheme Page
 *
 * @category: Magento
...
/**
 * Class CmsArtSliderAndBrandSchemeUpdate
 * @package Perficient\Base\Setup\Patch\Data
 */
class CmsArtSliderAndBrandSchemeUpdate implements DataPatchInterface
{
    /**
    ...
    public function apply(): void
    {
        ...
        $this->updateSourceofArtBlock();
        $this->updateBrandSchemesPage();
        $this->moduleDataSetup->endSetup();
    }

    /**
     * Update Source of Art Home page Block.
     */
    private function updateSourceofArtBlock(): void
    {
        $cmsBlock =
            [
 ...
 <div class="slider"></div>
        ...
        $updateCmsBlock = $this->blockRepository->getById($cmsBlock['identifier']);
        if (!$updateCmsBlock->getId()) {
            $updateCmsBlock->setData($cmsBlock)->save();
        } else {
            $updateCmsBlock->setContent($cmsBlock['content'])->save();
        }
    }

    /**
     * Update Brand Scheme Page.
     */
    private function updateBrandSchemesPage(): void
    {
        $updateCmsBrandSchemesPage = [
            [
                'title' => 'Brand Schemes',
                'stores' => [0],
...
{{widget type="Amasty\Customform\Block\Init" template="Amasty_Customform::init.phtml" form_id="6"}}</div></div></div>
        ...
        try {
            foreach ($updateCmsBrandSchemesPage as $cmsBrandSchemePage) {
                $myPageTemplate = $this->pageFactory->create()->load(
                    $cmsBrandSchemePage['identifier'],
                    'identifier'
                );

                if (!$myPageTemplate->getId()) {
                    $myPageTemplate->setData($cmsBrandSchemePage)->save();
                } else {
                    $myPageTemplate->setContent($cmsBrandSchemePage['content'])->save();
                }
            }
        } catch (\Exception $e) {
            throw $e;
