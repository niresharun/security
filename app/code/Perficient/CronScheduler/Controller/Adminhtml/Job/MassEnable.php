        ...
        try {
            foreach ($params as $jobCode) {
                if (in_array($jobCode, $disabledCrons)) {
                    unset($disabledCrons[array_search($jobCode, $disabledCrons)]);
                    $this->messageManager->addSuccessMessage(__('The job "%1" has been enabled.', $jobCode));
                }
            }
